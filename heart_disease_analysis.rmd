...
y.train <- data_train$TenYearCHD
y.test <- data_test$TenYearCHD
...
imp_test = mice.reuse(imp_train, data_test)
names(imp_test)
data_test_imp = imp_test$"1"

x.train_imp <- model.matrix(~.-1, data = data_train_imp)
x.test_imp <- model.matrix(~.-1, data = data_test_imp)
```

First we try the dataset where we imputed with our own technique. 
...
```{r Second Imputed Lasso model, eval = F}
...
imp2_test = mice.reuse(imp2_train, data_test)
data_test_imp_specified = imp2_test$"1"
...
```{r, eval = F}
lasso_imp_preds <- predict(lasso_mod_imp_s, newx = x.test_imp_s, type = "response",  s = cv.out$lambda.1se)
...
auc(roc_obj_lasso)
```
High Auc, but very unbalanced classification. Almost no patients are classified as positive, similar to the complete case logistic regression. Using different imputation methods seems to have some effect on the prediction, but the difference might be due to random effects.
