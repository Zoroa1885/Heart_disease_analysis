...
```
```{r, eval = F}
names(apply(B_coef_count, 2, sum)/B)[apply(B_coef_count, 2, sum)/B > 0.7]
```
...
colnames(B_coef) = names(coef(lasso_mod,s=cv.out$lambda.1se)[,1])
boxplot.matrix(B_coef[,-1], ylim = c(-0.4,0.5), las = 2)
#, ylim = c(-0.02,0.02)
...
The result form the imputed data set is quite different from that of the non-imputed one. We might considder to use all coeficients which used in over 70% of the models, which is in this case 
...
Total summary av hva vi har gjort og hva vi fant ut?
