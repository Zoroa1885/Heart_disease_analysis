...
This is our main objective, and to see that the variables really are significant, we fit a logistic model using the variables we have selected.
A secondary, although large, part of our project, is to explore methods for imputing missing data. 
We will do single regression imputation manually and through the 'mice' package, and investigate a bit what the Lasso is doing on the imputed data sets, compared to the complete case.

We start by examining the dataset. 


## Exploratory Analysis
```{r Loading Data, echo = F, fig.width= 4, fig.height=4, cex = 0.7}
...
```{r plot missing data, echo = F, fig.width= 10, fig.height=6, cex = 0.5}
# Look at the missing data
md_mat = md.pattern(data, rotate.names = T, plot = T)
```
```{r, echo = F, fig.width= 5, fig.height=3, cex = 0.5}
gg_miss_var(data)
```
...
```{r exp analysis, echo = F, fig.width= 5, fig.height=5, cex = 0.5}
...
```{r Remove rows with more than 1 NA, echo = F}
...
``` {r Missing data regression models, echo = F}
...
``` {r Missing data which-to-predict, echo = F}
...
``` {r Missing Data predictions, echo = F}
set.seed(8701)
...
```{r Missing data train-pred-plots, echo = F, fig.width=6, fig.height=4.5}
...
```{r Missing data train-pred-plots-2, echo = F, fig.width=6.5, fig.height=3.5}
...
``` {r Missing Data imputation, echo = F}
...
```{r Logistic Model, echo = F, fig.width= 4, fig.height=4}
...
```{r Lasso CV, echo = F, fig.height = 4.3 , fig.width=6}
...
plot(cv.out$glmnet.fit, "lambda", label=F, main = "CV for lambda - shrinkage of coefficients",  cex = 0.4)
plot(cv.out, main = "Binomial deviance for lambda")
#cv.out$lambda.min
#cv.out$lambda.1se
...
```{r Bootstrap Lasso, echo = F, fig.width=6, fig.height=4.2}
set.seed(8701)
...
# names_lasso_cc_50
```
We ran the bootstrap using `r B` iterations. The variables that have nonzero coefficients in the Lasso models at least 70\% of the time, are `r names_lasso_cc_70`.
...
```{r Manually Imputed Lasso model, echo = F, fig.height = 4.3 , fig.width=6}
...
```{r Manually Imputed Lasso model 2, echo = F, fig.height = 4 , fig.width=4}
...
```{r Mice Imputed Lasso model, echo = F, fig.height = 4.3 , fig.width=6}
set.seed(8701)
# Cross-validation
...
```{r Mice Imputed Lasso model 2, echo = F, fig.height = 4 , fig.width=4}
...
The performance of this model is quite similar to that of the complete case, which is to be expected.
The Lasso on the MICE-imputed data chooses `r names_coef_mice` as the significant covariates. 
...
```{r Bootstrap Lasso with imputed data, echo = F, fig.width=6, fig.height=4.2}
set.seed(8701)
B = 500
...
# names_lasso_mice_50
```
We ran the bootstrap using `r B` iterations. The variables that have nonzero coefficients in the Lasso models at least 70\% of the time, are `r names_lasso_mice_70`.
...
```{r complete case comparison, echo = F}
...
```{r Inference for imputed data, echo = F}
...
CI_mice = confint(mod)
cbind(as.matrix(summary(mod)$coefficients), as.matrix(CI_mice))
...
We first fit a logistic regression model on the complete case training set and then a Lasso on the complete case data set.
Bootstrapping was used to see how often each variable was included in a Lasso model. 
The variables that was present in at least $70$\% of the models, were `r names_lasso_cc_70`. 
As discussed in the exploratory analysis, some variables are included that are highly correlated to others. 
For example, 'sysBP' has been included in every model we ran, and as mentioned, 'sysBP' is extremely correlated to 'diaBP', which probably forced 'diaBP' out of the model.
If we select those variables that were in at least $50\%$ of the models, we obtain `r names_lasso_cc_50`, and we see that 'cigsPerDay' and 'prevalentHyp' is selected, as well.
This is as expected, as these were some of the variables we mentioned could absorb some of the information from omitted variables, such as 'currentSmoker' and 'diaBP'.

The Lasso on imputed data chose `r names_lasso_mice_70`, which matches the complete case quite well. 
...
The sensitivity and specificity of the went down slightly when using the imputed data, but as the Lasso selects the same variables, it does not matter a lot for our purposes.
The decrease in data quality should have been more visible if the percentage of imputed values were higher, or if we were in a not-so-data-rich setting.

What we can probably conclude, is that the variables `r names_lasso_cc_70` are the most significant, and that we may include 'cigsPerDay' or even 'prevalentHyp' if we want to.
Lesson learned: Try to avoid becoming an old, smoking man with high blood pressure, and let 'mice' impute your data instead of doing it manually. 
