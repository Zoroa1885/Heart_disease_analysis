...
X = model.matrix(TenYearCHD ~  .-1, data = df_complete)[,-1]
x.train_c = X[spl,]
x.test_c = X[-spl,]
y.train_c = df_complete$TenYearCHD[spl]
y.test_c = df_complete$TenYearCHD[-spl]
```

```{r exp analysis, eval = F}
# Make a correlation plot 
cor_mat2 = cor(subset(df_complete, select = -education)) 
cor_mat = cor(X)
ggcorrplot(cor_mat, hc.order = TRUE, type = "lower",
   outline.col = "white",
   colors = c("blue", "white", "red"))
ggcorrplot(cor_mat2, hc.order = TRUE, type = "lower",
   outline.col = "white",
   colors = c("blue", "white", "red"))
```
# Missing Data
...
#---------------------------------------------------------------------------------------------------------------
# Removing all samples that have more than one covariate missing and trying again, hopefully with better results
#---------------------------------------------------------------------------------------------------------------
...
fit_for_simp_glucose_2 <- lm(glucose ~., data = x.train_dat2)
...
fit_for_simp_cigs_2 <- lm(cigsPerDay ~., data = x.train_dat2)
...
fit_for_simp_edu_2 <- gknn(education ~., data = x.train_dat2)
...
There are several steps in this procedure that could be improved upon. First of all, we could have used more flexible models for imputation than linear imputation, and we could have fit several models on the training set and done an evaluation procedure within the training set (e.g. crossvalidated optimization of ROC-AUC) to pick the models imputing each variable. 
