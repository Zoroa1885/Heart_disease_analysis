...
auc(roc_obj_lasso)
```
The model performs significantly worse! It looks like the model uses variables like education as a proxy for a intercept, so we make a model with intercept to negate this effect
```{r First Imputed Lasso model}
