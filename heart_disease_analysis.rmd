...
# For training data
md.pattern(x.train_imp)
gg_miss_var(x.train_imp)

# For test data
md.pattern(x.test_imp)
...
X = model.matrix(TenYearCHD ~  .-1, data = df_complete) 
...
```{r}
df.test <- df_complete[-train,]

mod0_preds <- predict(mod0, newdata = df.test, type = "response")

sum(round(mod0_preds) == 1)

predicted_value <- factor(round(mod0_preds))
expected_value <- factor(df.test$TenYearCHD)

confusionMatrix(data=predicted_value, reference = expected_value)$table

roc_obj <- roc(df.test$TenYearCHD, mod0_preds)
auc(roc_obj)
```
...
cv.out = cv.glmnet(x.train_c, y.train_c, family = "binomial", intercept = F, standardize=TRUE, alpha = 1)
...
``` 
```{r Lasso Model}
lasso_mod = glmnet(x.train_c, y.train_c, family = "binomial", intercept = F, standardize=TRUE, alpha = 1,  lasso = cv.out$lambda.1se)
...
```{r Lasso Prediction}
lasso_preds <- predict(lasso_mod, newx = x.test_c, type = "response",  s = cv.out$lambda.1se)

sum(round(lasso_preds) == 1)

predicted_value <- factor(round(lasso_preds))
expected_value <- factor(y.test_c)

confusionMatrix(data=predicted_value, reference = expected_value)$table

roc_obj_lasso <- roc(y.test_c, lasso_preds)
auc(roc_obj_lasso)
```
...
y.train <- data_train_imp$TenYearCHD
y.test <- data_test_imp$TenYearCHD

data_train["TenYearCHD"] <- NULL
data_test["TenYearCHD"] <- NULL

imp_train = mice(data_train, m=1)
data_train_imp = complete(imp_train)

imp_test = mice(data_test, m=1)
data_test_imp = complete(imp_test)

x.train_imp <- model.matrix(~.-1, data = data_train_imp)
x.test_imp <- model.matrix(~.-1, data = data_test_imp)
```


```{r First Imputed Lasso model}
# We need to standardize the matrix so that we can drop intercepts.
cv.out = cv.glmnet(x.train_imp, y.train, family = "binomial", intercept = F, standardize=TRUE, alpha = 1)
plot(cv.out$glmnet.fit, "lambda", label=F)
plot(cv.out)
cv.out$lambda.min
cv.out$lambda.1se
```
```{r Imputed Lasso Model}
lasso_mod = glmnet(x.train_imp, y.train, family = "binomial", alpha = 1, intercept = F, standardize=TRUE,  lasso = cv.out$lambda.1se)

lasso_coef <- coef(lasso_mod,s=cv.out$lambda.1se)
lasso_coef[,1]
```
The model drops no variables, which is very worrisome. We check the predictive power of the model to see if this makes the model worse 

```{r}
lasso_preds <- predict(lasso_mod, newx = x.test_imp, type = "response",  s = cv.out$lambda.1se)
...
auc(roc_obj_lasso)
```
The model performs significantly worse! It looks like the model uses variables like education as a proxy for a intercept, so we make a model with intercept too remove this effect
```{r First Imputed Lasso model}
cv.out = cv.glmnet(x.train_imp, y.train, family = "binomial", alpha = 1)
plot(cv.out$glmnet.fit, "lambda", label=F)
plot(cv.out)
cv.out$lambda.min
cv.out$lambda.1se
...
```{r}
# We need to standardize the matrix so that we can drop intercepts.
lasso_mod = glmnet(x.train_imp, y.train, family = "binomial", alpha = 1,  lasso = cv.out$lambda.1se)

lasso_coef <- coef(lasso_mod,s=cv.out$lambda.1se)
lasso_coef[,1]
```
This time, the lasso drops most variables, like it did in the complete case.
```{r}
lasso_preds <- predict(lasso_mod, newx = x.test_imp, type = "response",  s = cv.out$lambda.1se)
...
auc(roc_obj_lasso)
```
The predictive performance is improved, but still worse than the complete case. This is probably because the model performs worse on data points with imputed values, since these values might not correlate correctly with the response.
### Specifying imputation methods

```{r Check what mice already uses}
...
```{r Specify new imputation methods}
meth["BPMeds"] <- "logreg"
...
```
```{r}
imp2_train = mice(data_train, m=1, method = meth)
data_train_imp_specified = complete(imp2_train)

imp2_test = mice(data_test, m=1, method = meth)
data_test_imp_specified = complete(imp2_test)

x.train_imp_s <- model.matrix(~.-1, data = data_train_imp_specified)
x.test_imp_S <- model.matrix(~.-1, data = data_test_imp_specified)
```

```{r Lasso Imputed With new methods}

# Use cross-validation to find lambda
cv.out = cv.glmnet(x.train_imp_s, y.train, family = "binomial", alpha = 1)
plot(cv.out$glmnet.fit, "lambda", label=F)
plot(cv.out)
cv.out$lambda.min
cv.out$lambda.1se
...
```{r}
lasso_mod_imp_s = glmnet(x.train_imp_s, y.train, family = "binomial", alpha = 1,  lasso = cv.out$lambda.1se)
...
```{r}
lasso_imp_preds <- predict(lasso_mod_imp_s, newx = x.test_imp, type = "response",  s = cv.out$lambda.1se)
...
auc(roc_obj_lasso)
```
Does not change the predictive performance.

### Bootstrapping

```{r Bootstrap Lasso with imputation in boot, eval = F}
set.seed(8701)
B = 500
size = dim(data)[1]
...
for (i in 1:B){
  ...
  data_b = data[boot_ind,]
  
  y = data_b$TenYearCHD
  data_b["TenYearCHD"] <- NULL
  
  imp_b <- mice(data_b, m = 1)
  data_b_imp = complete(imp_b)
  x = model.matrix(~.-1, data = data_b_imp)
...
colnames(B_coef) = names(coef(lasso_mod,s=cv.out$lambda.1se)[,1])
boxplot.matrix(B_coef[,-1], ylim = c(-0.25,0.55), las = 2)
```
```{r}
B_coef_count = ifelse(B_coef == 0,0,1)
barplot(apply(B_coef_count, 2, sum)/B, las=2)
...
```{r ootstrap Lasso with imputation outside boot, eval = F}
set.seed(8701)
B = 500
size = dim(data_train_imp)[1]

B_coef = matrix(NA, nrow = B, ncol = length(lasso_coef[,1]))
for (i in 1:B){
  boot_ind = sample(1:size, size = size, replace = TRUE)
  data_b = data_train_imp[boot_ind,]
  
  x = model.matrix(~.-1, data = data_b)
  y = y.train[boot_ind]
