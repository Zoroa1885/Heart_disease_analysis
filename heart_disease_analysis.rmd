...
```
```{r}
lasso_mod = glmnet(x.train_imp, y.train, family = "binomial", intercept = F, standardize=TRUE, alpha = 1,  lasso = cv.out$lambda.1se)
...
Specifying imputation methods. First we check what mice already uses.
```{r}
init = mice(data, maxit = 0)
meth = init$method
meth
```
Mice uses polyreg for the factor variable and predictive mean matching (pmm) for all other. Check data type
```{r}
...
predMat["TenYearCHD"] <- 0

imp = mice(data, m=1)
data_imp = complete(imp)
md.pattern(data_imp)

imp = mice(data, method = meth, predictorMatrix = predMat, m = 1)


X_train <- data[train,]
X_train[response] <- NULL

X_test <- data[-train,]
X_test[response] <- NULL

y.train <- data[response][train,]
y.test <- data[response][-train,]

imp_train <- mice(X_train, method = "norm.predict", m = 1)
imp_test <- mice(X_test, method = "norm.predict", m = 1)

x.train_imp <- complete(imp_train)
dim(x.train_imp)
?model.matrix
x.train_imp <- model.matrix(~.-1, x.train_imp)
dim(x.train_imp)

x.test_imp <- complete(imp_test)
x.test_imp <- model.matrix(~.-1, x.test_imp)

md.pattern(x.train_imp, rotate.names = TRUE)
```
...
cv.out$lambda.min
cv.out$lambda.1se
lasso_mod_imp = glmnet(x.train_imp, y.train, family = "binomial", intercept = F, standardize=TRUE, alpha = 1,  lasso = cv.out$lambda.1se)

lasso_imp_coef <- coef(lasso_mod_imp, s=cv.out$lambda.1se)
lasso_imp_coef[,1]
``` 

```{r}
lasso_imp_preds <- predict(lasso_mod_imp, newx = x.test_imp, type = "response",  s = cv.out$lambda.1se)
length(lasso_imp_preds)
sum(round(lasso_imp_preds) == 1)

predicted_value <- factor(round(lasso_imp_preds))
expected_value <- factor(y.test)
length(expected_value)
